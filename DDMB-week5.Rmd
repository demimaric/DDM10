...
ggplot(data = melted_cormat4, aes(x=Var1, y=Var2, fill=value)) + 
  geom_tile()+
  geom_text(aes(Var2, Var1, label = value), color = "white", size = 4)
...
cormat3 <- cor(df[df$country == 3, 159:165], use="pairwise.complete.obs")
cormat4 <- cor(df[df$country == 4, 159:165], use="pairwise.complete.obs")
>>>>>>> 7d53cc35d2bff41cf98a7ef057fab5d3d3a006b4
...
dfz <- cbind(na.omit(df$Country, df$pfi + df$itu + df$ip + df$ifl + df$unc + df$pu + df$peus))
