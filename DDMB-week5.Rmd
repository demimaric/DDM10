...
colnames(mydata)<-c("uncertainty","easeofuse", "Country")
dfu<-na.omit(df[,columns])
# generate data
N<-100
uncertainty  <- seq(min(dfu[,columns[2]]), max(dfu[,columns[2]]), length=N)
easeofuse  <- seq(min(dfu[,columns[3]]), max(dfu[,columns[3]]), length=N)
usefulness  <- seq(min(dfu[,columns[4]]), max(dfu[,columns[4]]), length=N)
image  <- seq(min(dfu[,columns[5]]), max(dfu[,columns[5]]), length=N)
intention  <- seq(min(dfu[,columns[6]]), max(dfu[,columns[6]]), length=N)
parents  <- seq(min(dfu[,columns[7]]), max(dfu[,columns[7]]), length=N)
